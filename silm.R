...
SimE.CI = function (X, Y, a_ind, n_ind, M = 1000, alpha = 0.95) 
{
  ...
  for (i in 1:length(a_ind)){
    ...
    a_band.st[i, ] <- cbind(beta.db[a_ind[i]] - a_crit.st/sqrt(n), beta.db[a_ind[i]] + a_crit.st/sqrt(n))
  }
  ...
  for (i in 1:length(n_ind)){
    ...
    n_band.st[i, ] <- cbind(beta.db[n_ind[i]] - n_crit.st/sqrt(n), beta.db[n_ind[i]] + n_crit.st/sqrt(n))
  }
