...
main_sim = function(s0, X_design, beta_design, err_design, g_design, nsim, seed_index, n_list, p_list, n_draws){
    for (i in 1:length(n_list)){
        ...
        cols = c("a_cov_rr_theta", "a_len_rr_theta", "n_cov_rr_theta", "n_len_rr_theta")
        ...
        write.csv(Results, file=sprintf("out/hdi_s_%d_x_%s_b_%s_e_%s_g_%s_r_%d_i_%d_n_%d_p_%d_d_%d_csv",
                                        s0, X_design, beta_design, err_design, g_design, nsim, (seed_index+1), n, p, n_draws))
        print(Results)
        print("--===- --=-=== ==--===")
    }
}
