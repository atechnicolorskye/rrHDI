...
## Dantzig M
riSingle_hdi_dantzig = function(y, X, M, a_ind, a_val, n_ind, n_val, g_design, scale_res) {
  ...
  A = matrix(0, nrow=t, ncol=p)          # s x p, binary indicator for H0_j
  for(i in 1:t) { A[i, test_ind[i]] = 1 }
  
  t0 = proc.time()[3]
  
  # test H0_j: beta_j = v0, for all j in test_ind
  ...
  An = (1/sqrt(n)) * A %*% M %*% t(X)   # (s + 15) x n.
  Tobs = t(sqrt(n) * (A %*% beta_dlasso - test_val))
  
  Tvals = matrix(0, nrow=0, ncol=t)     # R x (s + 15)
...
# riSingle_hdi_dantzig_backup = function(y, x, M, S_test, lam0_vals, g_design) {
#   # M = sparse M obtained via Dantzig selector
#   # Test H0_j: beta_j = lam0_j  simultaneously
#   # S_test = (id1, id2...) ids of variables to test (to  speed up things.)
#   #
#   n = nrow(x)
...
#   
#   returnList <- list("ci" = ci,"pvals" = Pvals)
#   return(returnList)
#   
# }
